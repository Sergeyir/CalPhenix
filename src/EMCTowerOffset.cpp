...
void EMCTiming::ProcessSector(const YAML::Node& sector)
{
   const std::string sectorName = sector["name"].as<std::string>();
   //const int numberOfYTowers = sector["number_of_towers_y"].as<int>();
   //const int numberOfZTowers = sector["number_of_towers_z"].as<int>();

   TFile inputFile(("./data/" + runName + "/EMCTiming/sum.root").c_str());
   TH3D *distrTVsADCVsZTower = static_cast<TH3D *>
      (inputFile.Get(("traw vs ADC vs iz: " + sectorName + ", iy0").c_str()));

   TF1 fitFunc("t vs ADC fit", inputYAMLCal["fit_func"].as<std::string>().c_str());

   for (int j = 1; j <= distrTVsADCVsZTower->GetZaxis()->GetNbins(); j++)
   {
      ...
      distrTVsADCVsZTower->GetZaxis()->SetRange(j, j);
                                          ...
                                          const int yTowerIndex, const int zTowerIndex)
{
   ...
   ROOTTools::PrintCanvas(&meanCanv, ("tmp/mean_iy" + std::to_string(yTowerIndex) + 
